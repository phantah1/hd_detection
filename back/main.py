...
import json
from flask import Flask, request, Response, send_file, abort, redirect
import os
...
if __name__ == "__main__":
    ...
    def upload_image():
        if request.files:
            reqimg = request.files["image"]
            imgstr = reqimg.read()
            root, ext = os.path.splitext(reqimg.filename)
            filename = root + "-detected" + ext
            filepath = os.path.join(os.path.dirname(
                os.path.abspath(__file__)), filename)
            npimg = np.fromstring(imgstr, np.uint8)
            img = cv2.imdecode(npimg, cv2.IMREAD_COLOR)
            count, overlay = recognize(img, odapi, threshold)
